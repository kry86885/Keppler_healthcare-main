...
export default function DailyMonthlyReportsPage({ setNotice, onNavigate }: Props) {
  ...
  const normalizeModeDisplay = (mode: string): string => {
    const map: Record<string, string> = {
      cash: "Cash", card: "Card", upi: "UPI", bank: "Bank Transfer",
    };
  ...
  const maxModuleVal = Math.max(...MODULES.map((m) => getAmountForModule(m.label)), 1);
  const maxMethodVal = Math.max(...PAYMENT_METHODS.map((m) => getAmountForMethod(m.name)), 1);
        ...
        <CardContent style={{ padding: "1.25rem" }}>
          <div style={{ display: "flex", gap: "1.5rem", alignItems: "flex-end", flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: "1rem", marginLeft: "auto" }}>
              <div style={{ padding: "0.875rem 1.5rem", borderRadius: "0.5rem", backgroundColor: "#f8fafc", border: "1px solid #e2e8f0", minWidth: "160px" }}>
              ...
              <div style={{ padding: "0.875rem 1.5rem", borderRadius: "0.5rem", backgroundColor: "#fef2f2", border: "1px solid #fee2e2", minWidth: "160px" }}>
        ...
        <CardContent>
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "0.5rem 0.75rem", backgroundColor: "#f8fafc", borderRadius: "0.375rem 0.375rem 0 0", borderBottom: "2px solid #e2e8f0" }}>
              <div style={{ width: "130px", fontSize: "0.7rem", fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em" }}>Module</div>
              <div style={{ flex: 1, fontSize: "0.7rem", fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em" }}>Progress</div>
              <div style={{ width: "80px", textAlign: "right", fontSize: "0.7rem", fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em" }}>Total</div>
            </div>
              ...
              {MODULES.map((mod, idx) => {
                ...
                return (
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => e.key === "Enter" && openModuleModal(mod.label)}
                    style={{
                      display: "flex",
                      alignItems: "center",
                      gap: "1rem",
                      padding: "0.875rem 0.75rem",
                      borderBottom: idx < MODULES.length - 1 ? "1px solid #e2e8f0" : "none",
                      cursor: "pointer",
                  ...
                  >
                    <div style={{ width: "130px", fontWeight: 600, fontSize: "0.875rem", color: "#0f172a" }}>{mod.label}</div>
                    <div style={{ flex: 1, backgroundColor: "#e2e8f0", height: "8px", borderRadius: "9999px", overflow: "hidden" }}>
                      <div style={{ width: `${pct}%`, backgroundColor: mod.color, height: "100%", borderRadius: "9999px", transition: "width 0.4s ease" }} />
                    </div>
                    <div style={{ width: "80px", textAlign: "right", fontWeight: 700, fontSize: "0.875rem", color: "#0f172a" }}>{formatCurrency(total)}</div>
        ...
        <CardContent>
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "0.5rem 0.75rem", backgroundColor: "#f8fafc", borderRadius: "0.375rem 0.375rem 0 0", borderBottom: "2px solid #e2e8f0" }}>
              <span style={{ fontSize: "0.7rem", fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em" }}>Payment Method</span>
              <span style={{ fontSize: "0.7rem", fontWeight: 700, color: "#475569", textTransform: "uppercase", letterSpacing: "0.05em" }}>Amount Collected</span>
            </div>
            <div style={{ border: "1px solid #e2e8f0", borderTop: "none", borderRadius: "0 0 0.5rem 0.5rem", overflow: "hidden" }}>
              {PAYMENT_METHODS.map((method, idx) => {
                ...
                return (
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => e.key === "Enter" && openMethodModal(method.name)}
                    style={{
                      display: "flex",
                      justifyContent: "space-between",
                      alignItems: "center",
                      padding: "0.875rem 0.75rem",
                      borderBottom: idx < PAYMENT_METHODS.length - 1 ? "1px solid #e2e8f0" : "none",
                      cursor: "pointer",
                      backgroundColor: "#ffffff",
                      transition: "background-color 0.15s",
                      userSelect: "none",
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: "0.875rem" }}>
                      ...
                      <div>
                        <span style={{ fontWeight: 600, fontSize: "0.875rem", color: "#0f172a", display: "block" }}>{method.name}</span>
                        <span style={{ fontSize: "0.75rem", color: "#94a3b8" }}>{txCount} transaction{txCount !== 1 ? "s" : ""}</span>
                      </div>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "0.625rem" }}>
                      <span style={{ fontWeight: 700, fontSize: "0.9rem", color: "#0f172a" }}>{formatCurrency(total)}</span>
                      <span style={{ color: "#94a3b8", fontSize: "1rem" }}>›</span>
                    </div>
                  </div>
                );
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "1.5rem", paddingTop: "1rem", borderTop: "2px solid #e2e8f0" }}>
              ...
              <span style={{ fontWeight: 800, fontSize: "1.125rem", color: "#0f172a" }}>{formatCurrency(totalCollected)}</span>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        {filteredRecords.length === 0 ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "3rem 1rem", gap: "0.75rem" }}>
            ...
            <p style={{ color: "#94a3b8", fontSize: "0.875rem", margin: 0, textAlign: "center" }}>
              No {period.toLowerCase()} transactions recorded for <strong>{selectedFilter}</strong>.<br />
          ...
          <Table>
            <thead>
              <TableRow>
                ...
                <TableCell>{filterType === "method" ? "Module" : "Payment Method"}</TableCell>
            ...
            <tbody>
              {filteredRecords.map((r, i) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell style={{ color: "#64748b" }}>{r.patientId || "—"}</TableCell>
                  <TableCell>{filterType === "method" ? r.module : r.paymentMethod}</TableCell>
