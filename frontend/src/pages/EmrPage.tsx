...
import { FiSearch as Search, FiPrinter as Printer, FiShare2 as Share2, FiZap as Zap, FiChevronLeft as Back } from "react-icons/fi";
...
const fmtDt = (ts: string) =>
  ts ? new Date(ts).toLocaleString("en-IN", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: true }) : "—";
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span style={{
      ...Object.fromEntries(palette[key].split(";").map(p => p.split(":") as [string,string])),
...
export default function EmrPage({ setNotice }: { setNotice: (msg: any) => void }) {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const res = await apiFetch<Patient[]>(`/api/emr/search?q=${encodeURIComponent(query)}`);
      setResults(res);
    } catch { setNotice({ type: "error", message: "Search failed." }); }
  ...
  const handleOpen = async (patientId: string) => {
    ...
    try {
      ...
      setData(d);
      apiFetch("/api/emr/access-log", { method: "POST", body: JSON.stringify({ patient_id: patientId, action: "viewed" }) }).catch(() => {});
    } catch { setNotice({ type: "error", message: "Failed to load EMR." }); }
    finally { setLoading(false); }
  };
  ...
  const handleAI = async () => {
    ...
    setLoading(true);
    try {
      const r = await apiFetch<any>(`/api/emr/${pid}/ai-summary`, { method: "POST" });
      setAiSum(r.summary);
    } catch { setNotice({ type: "error", message: "AI summary failed." }); }
    finally { setLoading(false); }
  };
  ...
  const handleWhatsApp = async () => {
    ...
    if (phone) {
      apiFetch("/api/whatsapp/init_feedback", {
        method: "POST",
        body: JSON.stringify({ patient_id: data.patient.patient_id || data.patient.id, phone }),
      }).catch(() => {});
    }
  ...
  if (pid && data) {
    const {
      ...
      encounters = [], diagnoses = [], prescriptions = [],
      labs = [], documents = [], pharmacy_sales = [], appointments = []
    } = data;
    ...
    const timeline = [
      ...encounters.map((e: any) => {
        ...
        return {
          ...
          desc: dx.length ? `Diagnosis: ${dx.map((d: any) => d.diagnosis_name).join(", ")}` : "Clinical encounter",
          ts: e.created_at, by: e.doctor_name || "Doctor", status: e.status || "Completed", amt: null,
        };
      }),
      ...appointments.map((a: any) => ({
        ...
        by: a.doctor_name || "—", status: a.status || "Scheduled",
        amt: a.consultation_fee > 0 ? Number(a.consultation_fee) : null,
      })),
      ...pharmacy_sales.map((s: any) => ({
        type: "Pharmacy",
        desc: `${s.medicine_name} × ${s.quantity}`,
        ts: s.sold_at || s.created_at, by: "Pharmacist", status: "Dispensed",
        amt: Number(s.amount || 0),
      })),
      ...documents.map((d: any) => ({
        type: "Document",
        desc: `${d.doc_type || "File"}: ${d.file_name}`,
        ts: d.created_at, by: "System", status: "Uploaded", amt: null,
      })),
    ].sort((a, b) => new Date(a.ts).getTime() - new Date(b.ts).getTime());

    const totalFee = appointments.reduce((s: number, a: any) => s + (a.consultation_fee > 0 ? Number(a.consultation_fee) : 0), 0);
    const totalPharm = pharmacy_sales.reduce((s: number, a: any) => s + Number(a.amount || 0), 0);
    ...
    return (
      <div style={{ padding: "24px 0" }}>
        ...
        {/* Top bar – screen only */}
        <div className="no-print" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", padding: "0 4px" }}>
          <button
            onClick={() => { setPid(null); setData(null); setAiSum(""); }}
            style={{ display: "flex", alignItems: "center", gap: "6px", background: "none", border: "1px solid #cbd5e1", borderRadius: "8px", padding: "8px 16px", cursor: "pointer", fontWeight: 600, color: "#374151" }}
          >
            <Back size={16} /> Back to Search
          </button>
          <div style={{ display: "flex", gap: "10px" }}>
            <button onClick={handleAI} style={{ display: "flex", alignItems: "center", gap: "6px", background: "#faf5ff", border: "1px solid #c4b5fd", borderRadius: "8px", padding: "8px 16px", cursor: "pointer", fontWeight: 600, color: "#5b21b6", fontSize: "14px" }}>
              <Zap size={16} /> AI Summary
            </button>
            <button onClick={() => setShowModal(true)} style={{ display: "flex", alignItems: "center", gap: "6px", background: "#25d366", border: "none", borderRadius: "8px", padding: "8px 18px", cursor: "pointer", fontWeight: 700, color: "#fff", fontSize: "14px" }}>
              <Share2 size={16} /> Share via WhatsApp
            </button>
            <button onClick={() => window.print()} style={{ display: "flex", alignItems: "center", gap: "6px", background: "#1e3a5f", border: "none", borderRadius: "8px", padding: "8px 16px", cursor: "pointer", fontWeight: 600, color: "#fff", fontSize: "14px" }}>
        ...
        <div id="emr-printable" className="rpt-wrap">
          ...
          <div className="rpt-header">
            <div>
              ...
              <div className="rpt-logo-sub">Advanced Healthcare Management System</div>
              <div className="rpt-doc-title">PATIENT JOURNEY REPORT</div>
            </div>
            <div className="rpt-meta">
              <div><strong>Report Date:</strong> {new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" })}</div>
              <div><strong>Generated At:</strong> {new Date().toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: true })}</div>
              <div><strong>Patient ID:</strong> {patient.patient_id}</div>
              <div><strong>System:</strong> HospAI EMR v2.0</div>
            </div>
          </div>
          ...
          <table className="info-grid">
            <tbody>
              <tr>
                ...
                <td><strong>{patient.name} {patient.last_name || ""}</strong></td>
              </tr>
              <tr>
          ...
          <table className="info-grid">
            <tbody>
              <tr>
                ...
                <td>{medical_history?.allergies || patient.allergies || "None"}</td>
          ...
          {aiSum && (
            <>
              ...
              <div style={{ border: "1px solid #c4b5fd", background: "#faf5ff", borderRadius: "0 0 6px 6px", padding: "12px 14px", fontSize: "12.5px", color: "#374151", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>
                {aiSum}
              </div>
            </>
          )}
                ...
                <tbody>
                  {appointments.map((a: any, i: number) => (
                    <tr key={i}>
                      ...
                      <td style={{ textAlign: "right", fontWeight: 700, color: a.consultation_fee > 0 ? "#065f46" : "#94a3b8" }}>
                        {a.consultation_fee > 0 ? `₹${Number(a.consultation_fee).toFixed(2)}` : "—"}
                      </td>
                      <td style={{ textAlign: "center" }}><StatusBadge status={a.status || "Scheduled"} /></td>
                    </tr>
                  ))}
                </tbody>
                {totalFee > 0 && (
                  <tfoot>
                    <tr>
                      <td colSpan={4} style={{ textAlign: "right" }}>Total Consultation Fees:</td>
                      <td style={{ textAlign: "right" }}>₹{totalFee.toFixed(2)}</td>
                      <td />
                    </tr>
                  </tfoot>
                )}
          ...
          {labs.length > 0 && (
            <>
              ...
              <table className="data-tbl">
                <thead>
                  <tr><th>#</th><th>Test Name</th><th>Result</th><th>Normal Range</th><th>Date</th><th style={{ textAlign: "center" }}>Status</th></tr>
                </thead>
                <tbody>
                  {labs.map((lab: any, i: number) => (
                    <tr key={i}>
                      ...
                      <td style={{ textAlign: "center" }}><StatusBadge status={lab.status || "Done"} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {pharmacy_sales.length > 0 && (
            <>
              ...
              <table className="data-tbl">
                <thead>
                  <tr><th>#</th><th>Medicine</th><th style={{ textAlign: "center" }}>Qty</th><th style={{ textAlign: "right" }}>Amount</th><th>Date</th><th style={{ textAlign: "center" }}>Status</th></tr>
                </thead>
                <tbody>
                  {pharmacy_sales.map((s: any, i: number) => (
                    <tr key={i}>
                      ...
                      <td style={{ textAlign: "right", fontWeight: 700, color: "#065f46" }}>₹{Number(s.amount).toFixed(2)}</td>
                      <td>{fmt(s.sold_at || s.created_at)}</td>
                      <td style={{ textAlign: "center" }}><StatusBadge status="Dispensed" /></td>
                    </tr>
                  ))}
                </tbody>
                {totalPharm > 0 && (
                  <tfoot>
                    <tr>
                      <td colSpan={3} style={{ textAlign: "right" }}>Total Pharmacy Charges:</td>
                      <td style={{ textAlign: "right" }}>₹{totalPharm.toFixed(2)}</td>
                      <td colSpan={2} />
                    </tr>
                  </tfoot>
                )}
          ...
          {/* ── JOURNEY TIMELINE ── */}
          <div className="sec-title">Journey Timeline — Chronological (Oldest to Newest)</div>
          <table className="data-tbl">
            <thead>
              <tr>
            ...
            <tbody>
              {timeline.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign: "center", padding: "20px", color: "#94a3b8" }}>No events found.</td></tr>
              ) : timeline.map((e: any, i: number) => (
                <tr key={i}>
                  <td style={{ color: "#64748b", fontWeight: 600, textAlign: "center" }}>{i + 1}</td>
                  <td style={{ fontWeight: 700, color: "#1e3a5f", fontSize: "12px" }}>{e.type}</td>
                  <td style={{ fontSize: "12px" }}>{e.desc}</td>
                  <td style={{ fontSize: "11.5px" }}>
                    <div style={{ fontWeight: 600 }}>{e.ts ? fmt(e.ts) : "—"}</div>
                    <div style={{ color: "#64748b" }}>{e.ts ? new Date(e.ts).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit", hour12: true }) : ""}</div>
                  </td>
                  <td style={{ fontSize: "12px" }}>{e.by}</td>
                  <td style={{ textAlign: "right", fontWeight: 700, color: e.amt != null ? "#065f46" : "#cbd5e1", fontSize: "12px" }}>
                    {e.amt != null ? `₹${Number(e.amt).toFixed(2)}` : "—"}
                  </td>
                  <td style={{ textAlign: "center" }}><StatusBadge status={e.status} /></td>
                </tr>
              ))}
            </tbody>
            {timeline.some((e: any) => e.amt != null) && (
              <tfoot>
                <tr>
                  ...
                  <td style={{ textAlign: "right" }}>₹{timeline.filter((e: any) => e.amt != null).reduce((s: number, e: any) => s + Number(e.amt), 0).toFixed(2)}</td>
                  <td />
                </tr>
              </tfoot>
            )}
          ...
          {grandTotal > 0 && (
            <>
              ...
              <div className="fin-box">
                {totalFee > 0 && <div className="fin-row sub"><span>Consultation Fees ({appointments.filter((a: any) => a.consultation_fee > 0).length} visit{appointments.length > 1 ? "s" : ""})</span><span style={{ fontWeight: 700, color: "#065f46" }}>₹{totalFee.toFixed(2)}</span></div>}
                {totalPharm > 0 && <div className="fin-row sub"><span>Pharmacy Charges ({pharmacy_sales.length} item{pharmacy_sales.length > 1 ? "s" : ""})</span><span style={{ fontWeight: 700, color: "#065f46" }}>₹{totalPharm.toFixed(2)}</span></div>}
                <div className="fin-row"><span>TOTAL AMOUNT PAYABLE</span><span>₹{grandTotal.toFixed(2)}</span></div>
              </div>
            </>
          )}
          ...
          <div className="rpt-footer">
            <span>HospAI Medical Centre — <em>Confidential Patient Record</em></span>
            <span className="rpt-footer-center">This is a system-generated document. No signature required.</span>
            <span>Printed: {new Date().toLocaleString("en-IN")}</span>
          </div>
        </div>

        {/* ══════════════ WHATSAPP MODAL ══════════════ */}
        <Modal open={showModal} onClose={() => setShowModal(false)} title="Share Patient Report via WhatsApp">
          <div className="wa-modal-body">
            <p style={{ fontSize: "13.5px", color: "#374151", marginBottom: "16px", lineHeight: 1.6 }}>
              Clicking the button below will:
              <br />
              <strong>1.</strong> Automatically open the <strong>Print / Save PDF dialog</strong> so you can save the report
              <br />
              <strong>2.</strong> Open <strong>WhatsApp</strong> with a personalised feedback message for <strong>{data?.patient?.name}</strong>
            </p>
            <button className="wa-big-btn" onClick={handleWhatsApp}>
              <svg viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.558 4.126 1.532 5.859L.057 23.776l6.079-1.594A11.942 11.942 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.812 9.812 0 01-5.007-1.372l-.359-.214-3.717.976.993-3.623-.234-.373A9.816 9.816 0 012.182 12c0-5.424 4.394-9.818 9.818-9.818 5.424 0 9.818 4.394 9.818 9.818 0 5.424-4.394 9.818-9.818 9.818z"/></svg>
              Send Report + Feedback Request on WhatsApp
            </button>
            <div className="wa-note">
              ✅ The PDF print dialog will open automatically first.<br />
              ✅ Save the PDF, then attach it to the WhatsApp message that opens.<br />
              ✅ The message includes a personalised feedback request for the patient.
            </div>
          </div>
        </Modal>
      </div>
    ...
    <section className="module-page">
      ...
      <div className="panel registration-desk-panel">
        <p className="muted" style={{ marginBottom: "16px" }}>
          Search patient by Name, last 4 digits of mobile number, UHID, or Patient ID.
        </p>
        ...
        {results.length > 0 && (
          <>
            <h3 style={{ fontWeight: 700, color: "#1e3a5f", marginBottom: "12px" }}>Search Results</h3>
            <table className="data-tbl">
              <thead>
                <tr>
                  <th>Patient ID</th>
              ...
              <tbody>
                {results.map(p => (
                  <tr key={p.id}>
                    <td style={{ fontWeight: 700, color: "#1e3a5f" }}>{p.patient_id}</td>
                    <td>{p.name} {p.last_name}</td>
                    <td>{p.phone}</td>
                    <td>{p.age} / {p.gender}</td>
                    <td>
                      <Button size="sm" variant="primary" onClick={() => handleOpen(p.patient_id)}>
                        Open EMR
                      </Button>
                    </td>
                  </tr>
        ...
        {results.length === 0 && query && !loading && (
          <div style={{ textAlign: "center", padding: "40px", color: "#94a3b8", background: "#f8fafc", borderRadius: "10px", border: "1px dashed #e2e8f0", marginTop: "12px" }}>
            <Search size={32} style={{ margin: "0 auto 8px", display: "block" }} />
