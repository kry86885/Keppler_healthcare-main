...
def get_emr(patient_id):
    ...
    with get_connection() as conn:
        ...
        # Clinical Notes
        cursor.execute(
            "SELECT * FROM clinical_notes WHERE patient_id = %s", (patient_id,)
        )
        notes = [dict(r) for r in cursor.fetchall()]

        # Vitals
        cursor.execute(
            "SELECT * FROM patient_vitals WHERE patient_id = %s", (patient_id,)
        )
        vitals = [dict(r) for r in cursor.fetchall()]

        # Diagnosis
        cursor.execute(
            "SELECT * FROM diagnosis_records WHERE patient_id = %s", (patient_id,)
        )
        diagnoses = [dict(r) for r in cursor.fetchall()]

        # Prescriptions
        cursor.execute(
            "SELECT * FROM pharmacy_prescriptions WHERE patient_id = %s AND hospital_id = %s",
            (patient_id, hospital_id),
        )
        ...
        # Labs (diagnostics)
        cursor.execute(
            "SELECT * FROM diagnostics WHERE patient_id = %s AND hospital_id = %s",
            (patient_id, hospital_id),
        )
        ...
        cursor.execute(
            "SELECT id, patient_id, doc_type, file_name, created_at, ocr_text FROM documents WHERE patient_id = %s",
            (patient_id,),
        )
        ...
        cursor.execute(
            "SELECT * FROM pharmacy_sales WHERE patient_id = %s AND hospital_id = %s",
            (patient_id, hospital_id),
        )
